...
zipkin_loc = urlparse(os.environ.get('ZIPKIN_URL')).netloc.split (":")
if 1 == len (zipkin_loc):
	zipkin_transport = SimpleHTTPTransport (zipkin_loc[0], 80)
else:
	zipkin_transport = SimpleHTTPTransport (zipkin_loc[0], int(zipkin_loc[1]))

zipkin_service_name = 'tcpdump2es_v3'
...
@zipkin_span(service_name=zipkin_service_name, span_name='write_es', transport_handler=zipkin_transport, sample_rate=10.00)
def write_es (writer, body):
	writer.bulk (body=body)

@zipkin_span(service_name=zipkin_service_name, span_name='process_file', transport_handler=zipkin_transport, sample_rate=100.00)
